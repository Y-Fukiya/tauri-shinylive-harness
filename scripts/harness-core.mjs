...
export const writeVerificationProcedure = async (config) => {
  const lines = [
    ...
    "## Phase 3 Commands",
    "",
    "1. `npm run phase3:preflight:info` for non-blocking readiness diagnostics; `npm run phase3:preflight` is a backward-compatible alias.",
    "2. `npm run phase3:preflight:internal:macos` before an unsigned internal macOS release candidate.",
    "3. `npm run tauri:build:app:no-sign` for an unsigned internal macOS release candidate.",
    "4. `npm run phase3:preflight:strict:macos` before a signed/notarized macOS release candidate.",
    "5. `npm run tauri:build:app` after Apple signing and notarization credentials are configured.",
    "6. `npm run phase3:package`",
    "7. `npm run phase3:preflight:internal:windows` before an unsigned internal Windows release candidate.",
    "8. `npm run tauri:build:windows:no-sign` for an unsigned internal Windows release candidate.",
    "9. `npm run phase3:preflight:strict:windows` before a signed Windows release candidate.",
    "10. `npm run tauri:build:windows` after Windows signing credentials or a signing command are configured.",
    "11. `npm run phase3:package:windows`",
    "12. `npm run local:audit:macos` or `npm run local:audit:windows`",
    "13. `npm run phase3:release-draft` after the release has been reviewed.",
    "14. `node scripts/harness.mjs verify-release --release release/` before publishing release artifacts.",
    "15. `node scripts/harness.mjs package-template` when publishing a reusable starter artifact.",
    "",
    "## Acceptance Criteria",
    "",
