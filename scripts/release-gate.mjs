...
export const buildSteps = ({ platform, internal }) => {
  ...
  tauriBuildStep,
  { name: "verify:static:strict:final", command: "npm", args: ["run", "verify:static", "--", "--strict"] },
  { name: "guard:phi:release:final", command: "npm", args: ["run", "guard:phi:release"] },
  { name: "verify:offline:final", command: "npm", args: ["run", "verify:offline"] },
  { name: "verify:e2e:final", command: "npm", args: ["run", "verify:e2e"] },
  { name: "audit:reproducibility:strict:final", command: "npm", args: ["run", "audit:reproducibility:strict"] },
  {
    name: "review:signoff:pending-review",
    command: "npm",
    args: ["run", "review:signoff", "--", "--status", "pending-review", "--decision", "not-reviewed"],
  },
  { name: "evidence:index:final", command: "npm", args: ["run", "evidence:index"] },
  packageStep,
