...
const createDmgFromAppBundle = async (config, appBundle, destination, { internalRelease = false } = {}) => {
  ...
  if (!internalRelease && process.env.APPLE_SIGNING_IDENTITY) {
...
const createValidationPack = async (config, assets, platform = "macos") => {
  ...
  const releaseSmokePlan = buildReleaseSmokePlan({ config, context, platform, releaseType, internalRelease });
  await writeFile(
  ...
  await writeFile(
    ...
    [
      ...
      ...(internalRelease
        ? [
            "- Release type: unsigned-internal-candidate",
            "- External distribution ready: no",
            "- Intended distribution: controlled internal evaluation only",
            "- Signing / notarization: not required for this internal candidate",
          ]
        : [
            "- Release type: signed-release-candidate",
            "- External distribution ready: requires local release audit pass",
            "- Intended distribution: signed release candidate review",
            "- Signing / notarization: required when configured for this platform",
          ]),
      "",
      "## Scope",
      "",
...
if (config.distribution.macBundles.includes("dmg")) {
  ...
  await createDmgFromAppBundle(config, appBundle, releaseDmg, { internalRelease });
}
