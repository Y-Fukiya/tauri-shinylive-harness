...
Turn the Spike/MVP into a reusable local-first template and CLI that can generate, verify, package, and prepare one or more bundled Shinylive/webR apps for macOS and Windows release candidates.
...
- Distribution proof: unsigned macOS `.app`/DMG/pkg and Windows NSIS installer builds.
- Phase 3 preflight: Apple signing, notarization, Windows code signing, GitHub release, and tooling readiness.
- Phase 3 package: platform release artifacts, checksums, release notes, SBOM/license evidence, screenshot evidence, data validation evidence, and validation pack.
...
- `npm run tauri:build:windows:no-sign` produces a Windows NSIS installer on Windows.
- Packaged `.app` health endpoint serves `dist` from bundled resources.
- Windows installer artifacts are collected by `npm run phase3:package:windows`.
...
- Signed Windows release builds pass when Windows code-signing credentials or signing command are configured.
